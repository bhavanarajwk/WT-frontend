...
export function ReportsPageClient() {
  ...
  }, [activeSection, hasManagerAccess, hasHrAccess, timelogSubTab]);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(id);
  }, [user, loadMyProfile, activeSection]);
  useEffect(() => {
    if (activeSection !== "profile" || !canAccessProfile) return;
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, canAccessProfile, requiresSelfOnboarding]);
  useEffect(() => {
    if (activeSection !== "employee") return;
    const id = window.setTimeout(() => {
      void (async () => {
        try {
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, onboardForm.band_id, onboardForm.department, onboardForm.role, onboardDepartments]);
  useEffect(() => {
    if (activeSection !== "allocation") return;
  ...
  useEffect(() => {
    if (activeSection !== "masters") return;
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasManagerAccess]);  useEffect(() => {
    if (activeSection !== "timelog" || requiresSelfOnboarding) return;
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, requiresSelfOnboarding]);
  useEffect(() => {
    if (activeSection !== "timelog" || timelogSubTab !== "team" || !hasManagerAccess) return;
    const id = window.setTimeout(() => {
      void (async () => {
        try {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, timelogSubTab, hasManagerAccess, loadManagerData]);
  useEffect(() => {
    if (!hasManagerAccess) return;
    if (activeSection !== "timelog" || timelogSubTab !== "team") return;
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, timelogSubTab, hasManagerAccess, selectedManagerProjectCode]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, requiresSelfOnboarding, loadAllocationsForHr]);
  ...
  }, [activeSection, timelogSubTab, teamTimelogEmailFilter, loadTimelogsForCurrentRole]);
  ...
  const loadBgvDashboardReport = useCallback(async () => {
    ...
    if (activeSection !== "reports-workforce" || !hasHrAccess) return;
    const id = window.setTimeout(() => {
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, loadAttritionReports]);  useEffect(() => {
    if (activeSection !== "reports-section-4" || !hasHrAccess) return;
    const id = window.setTimeout(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, loadSkillInventoryReport]);  useEffect(() => {
    if (activeSection !== "reports-section-6" || !hasHrAccess) return;
    const id = window.setTimeout(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, loadContractDistributionReport]);  useEffect(() => {
    if (activeSection !== "reports-section-7" || !hasHrAccess) return;
    const id = window.setTimeout(() => {
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, loadBgvDashboardReport]);  useEffect(() => {
    if (
      (activeSection !== "offboarding" &&
        activeSection !== "reports-section-3" &&
        activeSection !== "background-verification") ||
      !hasHrAccess
    ) {
      return;
    }
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess]);  useEffect(() => {
    if (activeSection !== "background-verification" || !hasHrAccess) return;
    ...
    return () => window.clearTimeout(id);
  }, [activeSection, hasHrAccess, bgvForm.emp_id]);  useEffect(() => {
